...
export default async function middleware(req: NextRequest) {
  ...
  // Skip authentication for compare2 routes
  if (pathnameWithoutLocale.startsWith('/compare2') || pathnameWithoutLocale.startsWith('/fab')) {
    console.log('<< middleware end, skipping auth for compare2/fab route');
  ...
  try {
    ...
    console.log('Auth session fetch failed, continuing without auth:', error);
    session = null;
  }
